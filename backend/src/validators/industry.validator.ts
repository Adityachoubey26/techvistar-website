/**
 * @file src/validators/industry.validator.ts
 * @description Native validator for Industries CMS entries including rich CMS parameters.
 */
...
export function validateIndustryInput(input: IndustryInput, isUpdate = false): any {
  ...
  if (!isUpdate || input.title !== undefined) {
    if (input.title === undefined || input.title === null || String(input.title).trim() === '') {
      errors.push({ field: 'title', message: 'Industry title is required' });
    }
  }
